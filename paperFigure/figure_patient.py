...
import os
"""
This function extracts and saves data for a specific patient (identified by `hadm_id`) from all CSV files 
in a given folder. The filtered data is saved into a new directory, with each file 
containing only the rows corresponding to the specified patient.
"""
def onePatientPage(hadm_id, folder_path, output_dir):
    """
    Parameters:
    - hadm_id (str): The unique identifier for the patient whose data needs to be extracted.
    - folder_path (str): The path to the folder containing the CSV files to be processed.
    - output_dir (str): The path to the output directory where the filtered CSV files will be saved.
    """
...
"""
This function generates a comprehensive visualization of a patient's medical data, including vital signs, 
laboratory tests, medications, surgeries, and examinations. The data is extracted from multiple CSV files 
stored in the specified folder path. The visualization is divided into five subplots arranged vertically, 
each focusing on a specific category of medical information.
"""
def plot_patient(folder_path):
    """
    Parameters:
    - folder_path (str): The path to the folder containing the CSV files with the patient's medical data.
    """
    # 读取数据
    df_vitals = pd.read_csv(os.path.join(folder_path, 'Vital_signs.csv',encoding='gbk'))
    ...
    df_lab = pd.read_csv(os.path.join(folder_path, 'Laboratory_test.csv'))
    ...
    df_med = pd.read_csv(os.path.join(folder_path, 'Medication.csv'))
    df_med = df_med[['medication_trade_name_en','starttime_base','endtime_base']]
    df_ord = pd.read_csv(os.path.join(folder_path, 'Orders.csv'))
    ...
    df_surgeries = pd.read_csv(os.path.join(folder_path, 'Surgical_record_front_page.csv'))
    ...
    df_exams = pd.read_csv(os.path.join(folder_path, 'Examination.csv'))
...
if __name__ == '__main__':
    onePatientPage("jinhuaNSICU", "IP06394", "OnePatient")
    plot_patient("OnePatient")
